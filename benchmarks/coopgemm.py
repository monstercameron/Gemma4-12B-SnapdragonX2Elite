"""De-risk coopmat on Adreno X2: run the fp16 64x64xK coopmat GEMM (vk/coopgemm.spv) via raw Vulkan,
...
exts = ["VK_KHR_cooperative_matrix", "VK_KHR_vulkan_memory_model", "VK_KHR_16bit_storage", "VK_KHR_shader_float16_int8"]
...
data = open(os.path.join(HERE, "vk/coopgemm.spv"), "rb").read()
...
vk.vkUpdateDescriptorSets(dev, 4, [wr(ba, 0, A.nbytes, 0), wr(bb, 1, B.nbytes, 0), wr(bc, 2, M*N*2, 0), wr(bd, 3, 16, 1)], 0, None)
...
run()
C = np.frombuffer(dn(mc, M*N*2), np.float16).astype(np.float32).reshape(M, N)
