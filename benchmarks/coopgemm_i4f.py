"""Validate + benchmark the int4-weight coopmat GEMM (vk/coopgemm_i4f.spv). int4 weights dequantized to
...
data = open(os.path.join(HERE, "vk/coopgemm_i4f.spv"), "rb").read()
...
def bench(M, N, K):
    ...
    Xf = (rng.standard_normal((M, K)) * 0.1).astype(np.float32); X16 = Xf
    ...
    by, my = mkbuf(M * N * 4); bd, md = mkbuf(16); upb(md, np.array([N, K, M, N // 8], np.uint32))
    ...
    vk.vkUpdateDescriptorSets(dev, 5, [wr(bx, 0, X16.nbytes, 0), wr(bw, 1, wp.nbytes, 0), wr(bs, 2, scb.nbytes, 0), wr(by, 3, M * N * 4, 0), wr(bd, 4, 16, 1)], 0, None)
    ...
    run(); C = np.frombuffer(dnb(my, M * N * 4), np.float32).reshape(M, N)
